        ...
        questions: [
            {
                ...
                a: "PLA (Polylactic Acid) is a biodegradable thermoplastic derived from renewable resources like corn starch. It's excellent for decorative items, prototypes, and models that won't be exposed to high heat (above 50°C)."
            },
            {
...
const FAQ: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (categoryId) {
            const category = FAQ_DATA.find(c => c.id === parseInt(categoryId));
            if (category) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[minmax(180px,auto)]">
                    {FAQ_DATA.map((category, index) => (
                        <ScrollAnimation key={category.id} animation="fade-in-up" delay={index * 50} className={category.gridClass}>
                            <div
                            ...
                            >
                                ...
                                <category.icon
                                    className="absolute -bottom-4 -right-4 w-32 h-32 text-teal-900 opacity-5 transform rotate-12 group-hover:rotate-0 group-hover:scale-110 transition-transform duration-500"
                                />

                                {/* Content */}
                                <div className="relative z-10 p-6 h-full flex flex-col justify-between">
                                    <div className="bg-white shadow-sm w-12 h-12 rounded-xl flex items-center justify-center backdrop-blur-sm mb-4">
                                        <category.icon className="text-teal-600 w-6 h-6" />
                                    </div>

                                    <div>
                                        <h3 className="text-2xl font-bold text-charcoal-800 mb-1 leading-tight">{category.title}</h3>
                                        <p className="text-gray-600 text-sm font-medium">{category.subtitle}</p>
                                    </div>

                                    <div className="flex items-center text-teal-700 text-sm font-bold mt-4 opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0 duration-300">
                ...
                {activeCategory && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div className="relative bg-white w-full max-w-3xl max-h-[90vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-fade-in-up">
                            ...
                            {/* Modal Header */}
                            <div className={`p-8 pb-6 ${activeCategory.gradient} text-charcoal-800`}>
                                <button
                                    onClick={() => setActiveCategory(null)}
                                    className="absolute top-4 right-4 p-2 bg-black/5 hover:bg-black/10 rounded-full transition-colors text-charcoal-800"
                                >
                                    <X size={20} />
                                </button>

                                <div className="flex items-center gap-4 mb-6">
                                    <div className="p-3 bg-white shadow-sm rounded-xl backdrop-blur-md">
                                        <activeCategory.icon size={32} className="text-teal-600" />
                                    </div>
                                    <div>
                                        <h2 className="text-3xl font-bold">{activeCategory.title}</h2>
                                        <p className="text-gray-600">{activeCategory.subtitle}</p>
                                    </div>
                                </div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-lg text-charcoal-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 transition-all"
                                    />
                                    <Search className={`absolute left-3 top-3.5 w-5 h-5 ${searchQuery || document.activeElement?.tagName === 'INPUT' ? 'text-charcoal-800' : 'text-gray-400'}`} />
                                </div>
                            </div>
